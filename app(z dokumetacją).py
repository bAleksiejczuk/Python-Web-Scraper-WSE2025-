...
Najważniejsze możliwości:
  - Wielowątkowe pobieranie stron (ThreadPoolExecutor) z kontrolą opóźnienia
    pomiędzy zapytaniami (rate limiting).
  - Ograniczenie przeszukiwania do domeny startowej (z obsługą aliasów
    "www" oraz bez "www").
  - Ekstrakcja linków wewnętrznych oraz treści tekstowej; pominięcie plików
    binarnych (np. PDF/JPG/ZIP) i linków specjalnych (mailto/tel/javascript).
  - Zapis wyników do plików tekstowych oraz zapis błędów.
  - Deduplikacja treści po zakończeniu crawlowania.
  - Interfejs graficzny (Tkinter) do uruchamiania, monitorowania i pobierania
    plików wynikowych.

Pliki wyjściowe:
  - teksty.txt: surowe teksty stron (sekcje rozdzielone separatorem)
  - teksty_unikalne.txt: wynik deduplikacji (usunięcie duplikatów linii
    w obrębie każdej sekcji)
  - all_links.txt: lista wszystkich zapisanych URL-i
  - error_links.txt: lista błędów pobierania / parsowania / walidacji linków

Uwagi implementacyjne:
  - Kod nie modyfikuje serwisu; wykonuje wyłącznie żądania GET i analizę HTML.
  - Parametr `delay` ogranicza tempo zapytań; nadal należy respektować regulaminy
    serwisów oraz plik robots.txt (nie jest obsługiwany automatycznie).
"""
...
# ============================================================================
class Config:
    """Konfiguracja uruchomieniowa aplikacji.

    Klasa agreguje parametry wpływające na sposób przeszukiwania serwisu:
    - `url`: adres startowy (seed URL)
    - `max_pages`: limit liczby przetwarzanych stron
    - `max_workers`: liczba wątków w puli (ograniczona do 50)
    - `delay`: minimalne opóźnienie pomiędzy żądaniami HTTP (>= 0.3 s)

    Parametry są walidowane i normalizowane tak, aby ograniczyć ryzyko
    przeciążenia zasobów oraz błędów wynikających z niepoprawnych danych wejściowych.
    """
    ...
    def normalize_url(self):
        """Normalizuje URL wejściowy.

        Jeżeli użytkownik poda adres bez schematu (np. "example.com"), metoda
        dodaje prefiks "https://".

        Metoda nie wykonuje połączenia sieciowego — jest to wyłącznie operacja na stringu.
        """
...
# ============================================================================
class DomainManager:
    """Zarządza dozwolonym zakresem domen.

    Na podstawie domeny URL startowego budowana jest lista aliasów domeny,
    uwzględniająca warianty z "www" oraz bez "www". Dzięki temu crawler może
    poprawnie rozpoznawać linki wewnętrzne w typowych konfiguracjach serwisów.

    Przykład:
      - URL startowy: https://example.com
      - Dozwolone domeny: ["example.com", "www.example.com"]
    """
    ...
    def is_allowed(self, domain):
        ...
        Args:
            domain: domena (netloc) URL-a, np. "example.com".

        Returns:
            True, jeśli domena należy do listy dozwolonych aliasów.
        """
...
# ============================================================================
class HTMLParser:
    """Parser HTML odpowiedzialny za ekstrakcję linków i treści.

    Wykorzystuje BeautifulSoup do:
      - wydobycia linków (<a href="...">) oraz ich normalizacji,
      - odfiltrowania linków zewnętrznych i plików binarnych,
      - ekstrakcji tekstu z zachowaniem podstawowego formatowania (nagłówki,
        akapity, listy).

    Klasa nie pobiera danych z internetu — operuje na przekazanym HTML.
    """
    ...
    def parse(self, url, html):
        """Parsuje HTML i zwraca linki, błędy oraz tekst.

        Args:
            url: adres źródłowy aktualnie przetwarzanej strony (używany do
                normalizacji linków względnych).
            html: kod HTML jako string.

        Returns:
            Tuple[list[str], list[str], str]:
                - links: lista linków wewnętrznych (absolutnych) do dalszego crawlowania,
                - errors: lista komunikatów o błędnych linkach / wyjątkach parsowania,
                - text: wyekstrahowany i oczyszczony tekst.
        """
    ...
    def _extract_links(self, source_url, soup):
        """Wydobywa linki z dokumentu HTML.

        Zasady:
          - ignorowane są linki specjalne: mailto/tel/javascript/#,
          - dozwolone są wyłącznie domeny zgodne z DomainManager,
          - pomijane są zasoby binarne (np. PDF, obrazy, archiwa),
          - linki względne są normalizowane (urljoin), a następnie czyszczone
            do postaci scheme://netloc/path.

        Returns:
            (links, errors) gdzie:
              - links: list[str]
              - errors: list[str] (tylko przypadki uznane za warte zarejestrowania)
        """
        links, errors = [], []
    ...
    def _extract_text(self, soup):
        """Ekstrahuje tekst z HTML z podstawowym formatowaniem.

        Usuwa elementy niewidoczne lub techniczne (script/style/head/...)
        i dodaje znaki nowych linii, aby uzyskać czytelny zapis w pliku tekstowym.

        Returns:
            Oczyszczony tekst jako string.
        """
...
# ============================================================================
class HTTPClient:
    """Klient HTTP odpowiedzialny za pobieranie stron.

    Warstwa odpowiedzialna wyłącznie za komunikację sieciową:
      - wykonuje żądania HTTP GET z nagłówkiem User-Agent,
      - stosuje opóźnienie `Config.delay` przed pobraniem (rate limiting),
      - weryfikuje typ odpowiedzi (tylko text/html).

    W przypadku przerwania pracy w GUI (`stop_event`) metoda `fetch` zwraca
    kontrolowany błąd, aby zakończyć przetwarzanie.
    """
    ...
    def fetch(self, url):
        """Pobiera stronę i waliduje odpowiedź.

        Args:
            url: adres do pobrania.

        Returns:
            Tuple[bool, str|None, str|None]:
              - success: czy pobranie i walidacja typu zawartości się powiodły,
              - content: treść HTML (tylko gdy success=True),
              - error_msg: opis błędu (tylko gdy success=False).
        """
...
# ============================================================================
class Storage:
    """Warstwa zapisu danych do plików.

    Klasa odpowiada za spójny, thread-safe zapis wyników z wielu wątków.
    W praktyce przechowuje dwa otwarte uchwyty plików:
      - teksty.txt: zawartość tekstowa stron (URL + tekst + separator),
      - all_links.txt: lista URL-i faktycznie zapisanych.

    Separator `SEP` (80 znaków "_") rozdziela sekcje w pliku tekstowym, co ułatwia
    późniejsze przetwarzanie (np. deduplikację).
    """
    SEP = "_" * 80
    ...
    def save_page(self, url, text):
        """Zapisuje wynik przetworzenia jednej strony.

        Operacja jest chroniona blokadą `self.lock`, aby równoległe wątki nie
        mieszały danych w plikach.

        Args:
            url: URL przetwarzanej strony.
            text: wyekstrahowany tekst.
    ...
    def save_errors(self, errors):
        """Zapisuje listę błędów do pliku error_links.txt.

        Args:
            errors: lista komunikatów o błędach.
        """
        if errors:
            try:
    ...
    def close(self):
        """Zamyka otwarte uchwyty plików."""
        self.texts_f.close()
        self.links_f.close()
    
    def get_file_size_mb(self, filename):
        """Zwraca rozmiar pliku w MB (float).

        Metoda pomocnicza wykorzystywana w podsumowaniu działania.
        """
        try:
...
# ============================================================================
class Stats:
    """Statystyki i stan przebiegu crawlowania.

    Przechowuje:
      - `visited`: zbiór URL-i już przetworzonych,
      - `queued`: zbiór URL-i wykrytych/zakolejkowanych,
      - `errors`: lista komunikatów o błędach,
      - `start`: znacznik czasu startu (do obliczania czasu działania).

    Wszystkie odczyty/zapisy są synchronizowane blokadą, aby klasa była
    bezpieczna w środowisku wielowątkowym.
    """
    def __init__(self):
        self.visited = set()
        self.queued = set()
    ...
    def mark_visited(self, url):
        ...
        Returns:
            True, jeśli URL nie był wcześniej odwiedzony i został dodany.
            False, jeśli URL był już w zbiorze visited.
        """
        with self.lock:
    ...
    def add_queued(self, urls):
        """Dodaje nowe URL-e do zbioru zakolejkowanych.

        Metoda filtruje URL-e, które są już odwiedzone lub już zakolejkowane.

        Returns:
            Lista URL-i faktycznie dodanych w tym wywołaniu.
        """
        added = []
        with self.lock:
            for url in urls:
    ...
    def add_errors(self, errors):
        """Dodaje listę błędów do historii."""
        with self.lock:
            self.errors.extend(errors)
    
    def add_error(self, error):
        """Dodaje pojedynczy błąd do historii."""
        with self.lock:
            self.errors.append(error)
    
    def get_counts(self):
        """Zwraca liczniki (visited, queued, errors)."""
        with self.lock:
            return len(self.visited), len(self.queued), len(self.errors)
    
    def get_elapsed_time(self):
        """Zwraca czas działania od startu (w sekundach)."""
        return time.time() - self.start
    
    def get_errors(self):
        """Zwraca kopię listy błędów (thread-safe)."""
        with self.lock:
...
# ============================================================================
class Crawler:
    """Główny silnik crawlowania.

    Odpowiada za koordynację całego procesu:
      - inicjalizuje komponenty (DomainManager, HTTPClient, HTMLParser, Storage, Stats),
      - utrzymuje kolejkę URL do odwiedzenia,
      - uruchamia przetwarzanie wielowątkowe z wykorzystaniem ThreadPoolExecutor,
      - egzekwuje warunki zakończenia (limit stron, brak nowych linków, przerwanie użytkownika).
    """
    def __init__(self, config, stop_event=None):
        self.config = config
        self.stop_event = stop_event
    ...
    def run(self):
        """Uruchamia proces crawlowania.

        Metoda zarządza pulą wątków i przydziela zadania do pobrania oraz
        przetworzenia kolejnych URL-i. Wyniki są zapisywane na bieżąco do plików.

        Zakończenie następuje gdy:
          - osiągnięto `Config.max_pages`, lub
          - brak jest URL-i w kolejce i brak aktywnych zadań, lub
          - użytkownik przerwie działanie z GUI.
        """
    ...
    def _process(self, url):
        ...
        Kroki:
          1) deduplikacja URL (visited),
          2) pobranie HTML (HTTPClient),
          3) parsowanie i ekstrakcja (HTMLParser),
          4) zapis tekstu (Storage),
          5) dodanie nowych linków do kolejki (Stats.add_queued).

        Returns:
            Lista nowych URL-i (linków wewnętrznych) do dalszego przetwarzania.
        """
    ...
    def _print_summary(self):
        """Drukuje podsumowanie przebiegu oraz zapisuje błędy (jeśli wystąpiły)."""
...
class ConsoleRedirect:
    """Przekierowuje standardowe wyjście (print) do widgetu Tkinter.

    Obiekt jest podstawiany pod `sys.stdout`, dzięki czemu komunikaty
    wypisywane przez logikę crawlowania pojawiają się w oknie logów.
    """
...
class GUI:
    """Interfejs graficzny aplikacji (Tkinter).

    Zapewnia:
      - wprowadzenie URL i parametrów przetwarzania,
      - start/stop procesu (z wykorzystaniem `Event`),
      - podgląd logów (przekierowany stdout),
      - podgląd statystyk live,
      - zapis/pobranie plików wynikowych.

    Implementacja uruchamia crawling w osobnym wątku, aby nie blokować pętli
    zdarzeń Tkinter.
    """
    ...
    def start(self):
        """Uruchamia proces crawlowania po stronie GUI.

        Metoda:
          - waliduje parametry wprowadzone przez użytkownika,
          - przygotowuje stan UI (blokada przycisku Start, uruchomienie progressbaru),
          - przekierowuje stdout do okna logów,
          - uruchamia wątek roboczy wykonujący `Crawler.run()`.

        Uwaga: sama logika crawlowania działa w osobnym wątku, dzięki czemu GUI
    ...
    def stop(self):
        """Wysyła żądanie przerwania działania.

        Ustawia `stop_event`, co powoduje kontrolowane zakończenie pracy
        w `HTTPClient.fetch` oraz pętli głównej `Crawler.run`.
        """
        if self.running:
            self.stop_event.set()
    ...
    def _run(self, config):
        """Wątek roboczy uruchamiany z GUI.

        Tworzy instancję `Crawler`, uruchamia przetwarzanie oraz (gdy proces nie
        został przerwany) uruchamia deduplikację. Aktualizacja UI wykonywana jest
        poprzez `root.after`.
        """
        try:
